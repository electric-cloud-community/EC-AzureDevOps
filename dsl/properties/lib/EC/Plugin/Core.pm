...
sub get_config_values {
    ...
    if (!$self->{_config} || $renew) {
        ...
        my $execute_without_abort_error = sub {
            my $code = shift;
            # Saving
        ...
        for my $node ($properties->findnodes('//property')) {
            ...
            if ($name =~ /token_credential/) {
                eval {
                    $execute_without_abort_error->(sub {
                        my $credentials = $self->ec->getFullCredential($config_name . '_token_credential');
                        my $password = $credentials->findvalue('//password')->string_value;
                        $retval->{token} = $password;
                    });
                } or do {
