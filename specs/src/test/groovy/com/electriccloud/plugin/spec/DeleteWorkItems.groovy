...
class DeleteWorkItems extends PluginTestHelper {
    ...
    @Unroll
    def '#caseId. Smoke. Delete Single'() {
        given:
    ...
    @Unroll
    def '#caseId. Smoke. Delete Multiple'() {
        given:
        def workItemIdsArr = []
        [1, 2, 3].each {
            JSON itemToDelete = tfsClient.createWorkItem('Feature', [
    ...
    @Unroll
    def '#caseId. Smoke. Delete unexisting'() {
        given:
        def workItemIdsArr = []
        [1, 2, 3].each {
            JSON itemToDelete = tfsClient.createWorkItem('Feature', [
        ...
        // Deleting one of the work items, so it does not exist
        tfsClient.deleteWorkItem(workItemIdsArr[0])
        ...
        if (resultFormat != 'json') {
            String deletedItemId = deletedItemsHash['workItemIds']
            assert deletedItemId == workItemIdsArr[1..2].join(', ')
        }
