...
class UploadWorkItemAttachment extends PluginTestHelper {
    ...
    @Unroll
    def '#caseId. Smoke. Upload Type: "#uploadType", ContentSource: "#sourceType"'() {
        given:
    ...
    def '#caseId. Smoke. Upload Type: "chunked", ContentSource: "#sourceType"'() {
        given:

        String resultPropertyName = 'myAttachment'
        String resultPropertySheet = '/myJob/' + resultPropertyName
        ...
        fileContent = ''
        println("File created at $filePath")
        attachmentComment = randomize("Test comment")
        ...
        String attachmentUrl = getJobProperties(result.jobId)[resultPropertyName]['url']
        assert attachmentUrl

        def updatedWorkItem = tfsClient.getWorkItemById(workItemId, ['$expand': 'relations'])
        def relations = updatedWorkItem['relations']

        assert relations[0]
        assert relations[0]['attributes']['resourceSize'] == fileSizeKB * 1024
