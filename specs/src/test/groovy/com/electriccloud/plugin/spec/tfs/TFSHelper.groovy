...
class TFSHelper {
    ...
    JSON getWorkItemById(def id, Map params = [:]) {
        assert id
        String path = this.collectionName + '/_apis/wit/workitems/' + id
        return request(METHOD_GET, path, params)
    }
    ...
    JSON waitForBuild(int buildId, int timeout = 300, int waited = 0){
        ...
        if (result.status =~ /inProgress|notStarted/){
            ...
            return waitForBuild(buildId, timeout, waited + 10)
        }
    ...
    int getQueueIdByName(String queueName) {
        String path = [this.collectionName, this.projectName, '_apis/distributedtask/queues'].join('/')
        Map parameters = [
            queueName : queueName,
        ...
        JSON searchResult = this.client.get(url_with_params)

        if (searchResult.count == 0) {
            PluginTestHelper.logger.debug(searchResult.toString())
            throw new RuntimeException("Cannot find Queue with name ${queueName}.")
        }
        if (searchResult.count > 1) {
            PluginTestHelper.logger.debug(searchResult.toString())
            throw new RuntimeException("Search query returns more than one result.")
        }
